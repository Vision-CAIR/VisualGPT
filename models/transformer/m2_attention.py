...
class ScaledDotProductAttention_m2(nn.Module):
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h):
        ...
        '''
        super(ScaledDotProductAttention_m2, self).__init__()
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        if attention_mask is not None:
            att = att.masked_fill(attention_mask, -10000.0)
...
class ScaledDotProductAttentionMemory_m2(nn.Module):
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h, m):
        ...
        '''
        super(ScaledDotProductAttentionMemory_m2, self).__init__()
        self.fc_q = nn.Linear(d_model, h * d_k)
        self.fc_k = nn.Linear(d_model, h * d_k)
        self.fc_v = nn.Linear(d_model, h * d_v)
        self.fc_o = nn.Linear(h * d_v, d_model)
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        if attention_mask is not None:
            att[:, :, :, :nk] = att[:, :, :, :nk].masked_fill(attention_mask, -10000.0)
        att = torch.softmax(att, -1)
        out = torch.matmul(att, v).permute(0, 2, 1, 3).contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v)
        out = self.fc_o(out)  # (b_s, nq, d_model)
        return out
...
class MultiHeadAttention_m2(Module):
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, identity_map_reordering=False, can_be_stateful=False,
        ...
        super(MultiHeadAttention_m2, self).__init__()
