...
class Block(Module):
    ...
    def forward(self, x, layer_past=None,mask_queries=None,encoder_output=None,mask_encoder=None, mask_self_attention=None,iteration=0, encoder_decoder_tag= True):
        # print(x.shape, "x shape")
        # print(encoder_output.shape,"encoder output shape")
        # print(mask_encoder.shape, "mask encoder shape")
        ...
        # if encoder_decoder_tag:
        a_encoder = self.enc_dec_attn(x=self.ln_1(a), encoder_output=self.ln_1(encoder_output[:, 2]),mask_encoder=mask_encoder)
        a = a + a_encoder
        ...
        encoder_result = a + m
        # before, I calcualte the resid drop on a, but it should be the encoder_result
        # a = self.resid_pdrop(a)
        ## after the change
...
class GPT2Model(Module):
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, past=None,mask_queries=None,encoder_output=None,mask_encoder=None, mask_self_attention = None, encoder_decoder_tag=True):


        #
        # if self._is_stateful:
        #     past_length = int(self.running_seq[0])
        ...
        for block, layer_past in zip(self.h, past):
            iteration_index+=1
            hidden_states, present = block(hidden_states, layer_past,mask_queries = mask_queries,encoder_output=encoder_output,mask_encoder=mask_encoder, mask_self_attention= mask_self_attention, iteration=iteration_index,encoder_decoder_tag=encoder_decoder_tag)
...
class GPT2LMHeadModel(Module):
    ...
    def forward(self, input_ids, encoder_output=None, mask_encoder=None, position_ids=None, token_type_ids=None, lm_labels=None, past=None,encoder_decoder_tag=True):
        ...
        hidden_states, presents = self.transformer(input_ids, position_ids, token_type_ids, past,mask_queries=mask_queries,encoder_output=encoder_output,mask_encoder=mask_encoder, mask_self_attention= mask_self_attention, encoder_decoder_tag=encoder_decoder_tag)
