...
class Enc_Dec_Attention(Module):
    ...
    def forward(self, x, layer_past=None,encoder_output=None, mask_encoder=None):
        ...
        #
        # #
        # encoder_output = self.c_attn(encoder_output)
        # encoder_query, encoder_key, encoder_value = encoder_output.split(self.split_size,dim=2)
        # encoder_query =self.split_heads(encoder_query)
        # encoder_key = self.split_heads(encoder_key,k=True)
        # encoder_value = self.split_heads(encoder_value)
...
class Block(Module):
    ...
    def forward(self, x, layer_past=None,mask_queries=None,encoder_output=None,mask_encoder=None, mask_self_attention=None,iteration=0):
